...
def CAE_AE_TRAIN(shapes,task_samples,iterations):
    ...
    for batch_idx in range(1,iterations):
        ...
        if batch_idx %1==0:
            ...
            for i in range(0,len(task_samples)):
                ...
                final_weights=helper_functions.unFlattenNetwork(sample*SUM[i], shapes)
    ...
    final_dataframe_1.to_hdf('MSE_Iter/'+str(len(task_samples))+'_MSE_fixed_iter','key1')
    return accuracies
...
if Flags['mnist_train']:
    for mnsit_class in range(0,10):
        ...
        # model=model_reset
        if mnsit_class>=6:
            load_individual_class([mnsit_class],[1,2,3,4])
        else:
            load_individual_class([mnsit_class],[6,7,8,9])
        accuracy=TRAIN_TEST_MNIST()
        Actual_Accuracy.append(int(accuracy))
        print("Actual acc",Actual_Accuracy)
        torch.save(model.state_dict(),'mnist_individual_weights/mnist_digit_solver_'+str(mnsit_class)+'.pt')
