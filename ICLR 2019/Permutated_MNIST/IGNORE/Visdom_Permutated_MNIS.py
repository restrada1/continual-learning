...
# TRAIN
######################################################################################################
def train(args, model, device, train_loader, optimizer, epochs,task_number):
...
# TRAIN AND TEST
######################################################################################################
def TRAIN_TEST_MNIST(task_number):
    # for epoch in range(0, 3):
    acc=train(args, model, device, train_loader, optimizer, 3,task_number)
    #acc=test(args, model, device, test_loader)
    return acc
...
Skill_Mu=[]
running_update_count=[0 for _ in range(0,10)]
...
for permuatation in range(0,10):
    ...
    SHOW_TEST_TRAIN_IMAGES_SAMPLE()
    accuracy=TRAIN_TEST_MNIST('MNIST Skill '+str(permuatation))
    Actual_Accuracy.append(int(accuracy))
    print("Actual acc",Actual_Accuracy)
    torch.save(model.state_dict(),'../mnist_individual_weights/permutated_MNIST/mnist_reset_digit_solver_'+str(permuatation)+'.pt')
    if permuatation==0 :
