...
def load_individual_class(postive_class,negative_classes):
    ...
    for i in range(0,len(negative_classes)):
        index_train_negative=index_train_negative+train_Indexs[negative_classes[i]][0:int(0.3*(len(train_Indexs[negative_classes[i]])))]
...
def CAE_AE_TRAIN(shapes,task_samples,iterations):
    ...
    final_dataframe_1.to_hdf('MSE_Fixed_Iter/'+str(len(task_samples_copy))+'_MSE_fixed_iter','key1')
    return accuracies
#####################################################################################################################
...
for cifar_class in range(0,10):
    ...
    with open('MSE_Fixed_Iter/allAcc_resendskills_fixed_iterations_tasks_'+str(cifar_class), 'wb') as fp:
        pickle.dump(allAcc, fp)

    with open('MSE_Fixed_Iter/Actual_Accuracy_resendskills_fixed_iterations_tasks_'+str(cifar_class), 'wb') as fp:
        pickle.dump(Actual_Accuracy, fp)

    with open('MSE_Fixed_Iter/threshold_fixed_iterations_tasks_'+str(cifar_class), 'wb') as fp:
