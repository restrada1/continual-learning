...
def load_individual_class(postive_class,negative_classes):
    ...
    for i in range(0,len(negative_classes)):
        ...
        index_test_negative=index_test_negative+test_Indexs[negative_classes[i]][0:int(0.3*(len(test_Indexs[negative_classes[i]])))]
...
vae_optimizer = optim.Adam(student_model.parameters(), lr = 0.001)#, amsgrad=True)
lam = 0.001
Actual_Accuracy=[]
threshold_batchid=[]
    ...
    for batch_idx in range(1,iterations):
        ...
        for s in randPerm:
            ...
            loss,con_mse,con_loss,closs_mul_lam = helper_functions.Contractive_loss_function(W, 
                data.view(-1, 61326), recons_x,hidden_representation, lam)
