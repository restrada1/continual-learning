...
def FLATTEN_WEIGHTS_TRAIN_VAE(task_samples,model):
    ...
    if len(task_samples) < 7:
        CAE_AE_TRAIN(net_shapes,task_samples+final_skill_sample,1000)
    else:
        CAE_AE_TRAIN(net_shapes,task_samples+final_skill_sample,1000)
...
def CAE_AE_TRAIN(shapes,task_samples,iterations):
    ...
    for batch_idx in range(1,iterations):
        ...
        # print("running_thresholds: ", running_thresholds)
        ...
        # curr_batch_avg_loss = avg_loss 
        # curr_batch_max_loss = batch_losses[losses_wrt_thresholds.index(max(losses_wrt_thresholds))]
        ...
        # if np.all([x.data.numpy() for y in tmp_running_losses] < running_thresholds[:-(10-len(task_samples))]):
        # if curr_batch_avg_loss < initial_fmse_threshold:
        ...
        if batch_idx %999==0:  
            ...
            for i in range(0,len(task_samples)):
                ...
                print("RELATIVE PERFORMANCE TO ORIGINAL: "+str(round((Avg_Accuracy/Actual_Accuracy[i])*100, 2))+"%\n")
