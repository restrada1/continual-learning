...
#############################################################################
# FISHER-WEIGHTED MSE and Contractive Loss
#############################################################################
def Contractive_FMSE(W, x, recons_x, h, lam, fisher_vector):
    """Compute the Contractive AutoEncoder Loss with Fisher-Weighted MSE
    """
    
    #####################################################################
    #start new block
    ###################################################################
    f = np.interp(fisher_vector, (fisher_vector.min(), fisher_vector.max()), (1, 2))
    f = torch.FloatTensor(f)
    diff = torch.sub(recons_x, x)
    diff_squared = torch.pow(diff, 2).reshape(21432)
    fisher_exp = torch.pow(diff_squared, f)
    fisher_mse = torch.mean(fisher_exp)
    # fisher_mse = torch.dot(f, diff_squared)
    #####################################################################
    #end new block
    ###################################################################
    ...
    w_sum = w_sum.unsqueeze(1) # shape N_hidden x 1
    contractive_loss = torch.sum(torch.mm(dh**2, w_sum), 0)
    return fisher_mse+ contractive_loss.mul_(lam) ,fisher_mse,contractive_loss,contractive_loss.mul_(lam)  

#############################################################################
# FMSE WITH FEATURE REGRESSION  (JUST TESTING, NOT WORKING or NOT BENEFICIAL FOR NOW)
#############################################################################
def regression_FMSE(W, x, recons_x, h, lam, fisher_vector, recons_f, f):
    """Compute the Contractive AutoEncoder Loss with Fisher-Weighted MSE
    """
    
    #####################################################################
    #start new block (fmse)
    ###################################################################
    fsh = np.interp(fisher_vector, (fisher_vector.min(), fisher_vector.max()), (1, 2))
    fsh = torch.FloatTensor(fsh)
    diff = torch.sub(recons_x, x)
    diff_squared = torch.pow(diff, 2).reshape(21432)
    fisher_exp = torch.pow(diff_squared, fsh)
    fisher_mse = torch.mean(fisher_exp)
    # fisher_mse = torch.dot(f, diff_squared)
    #####################################################################
    #end new block (fmse)
    ...
    #####################################################################
    #start new block (f-regression)
    ###################################################################
    if type(f) != type([]):
        diff = torch.sub(recons_f, f)
        diff_squared = torch.pow(diff, 2).reshape(20)
        f_regression = torch.mean(diff_squared)
    #####################################################################
    #end new block (f-regression)
    ###################################################################
    ...
    if type(f) == type([]):
        total_loss = fisher_mse 
    else: 
        total_loss = fisher_mse + f_regression.mul_(.001)
    return total_loss,fisher_mse,contractive_loss,contractive_loss.mul_(lam)    
